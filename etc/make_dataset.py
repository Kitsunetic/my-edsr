...
dst_dataset_path = '/mnt/d/datasets/SRRAW4000'
  ...
  for _ in range(10):
    ...
    with tqdm(total=dataset_size, desc='copy files', unit='file') as t:
      for i, (lr_path, hr_path) in enumerate(src_paths, 1):
        ...
        # get color mean
        color_mean[0] += lr[:, :, 0].mean() / dataset_size / 10
        color_mean[1] += lr[:, :, 1].mean() / dataset_size / 10
        color_mean[2] += lr[:, :, 2].mean() / dataset_size / 10
        color_mean[3] += lr[:, :, 3].mean() / dataset_size / 10
        
        dst_path = os.path.join(dst_dataset_path, '%05d.pkl'%(dataset_size*_ + i))
